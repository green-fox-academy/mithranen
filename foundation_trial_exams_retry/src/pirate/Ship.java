...
//Can't add the same pirate with the same value!!!!
      if (pirate.equals(crewPirate)) {
        System.err.println("This pirate is already on the ship!");
        return;
      }
    }
...
//     then the ship's lastDayOnTheShip() method.
  ...
  //TODO baromság
/*  public void prepareForBattle(Pirate pirate) {
    if (pirate.equals(this.captain)) {
      for (int i = 0; i < 5; i++) {
        captain.work();
      }
    }
    if (pirate.equals(this.pirates)) {
      for (int i = 0; i < 5; i++) {
        pirate.work();
      }
    }
